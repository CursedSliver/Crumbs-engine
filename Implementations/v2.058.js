(function() {
	...
	Crumbs.objectBehaviors.cookieFadeout = new Crumbs.behavior(function(p) {
		this.alpha = Math.pow((this.t - Crumbs.t) / p.speed, 0.5);
	}, { speed: 1 /0.05 });
	...
	Crumbs.spawnFallingCookie = function(x, y, yd, xd, t, id, onMouse, sc, order, noInit, iconInput) {
		...
		let c = {
			behaviors: [
				new Crumbs.behaviorInstance(Crumbs.objectBehaviors.cookieFall, {yd: (yd || (4+Math.random()*2)) }), 
	...
	eval('Game.ClickSpecialPic='+Game.ClickSpecialPic.toString().replace(`Game.particleAdd(Game.mouseX,Game.mouseY-32,Math.random()*4-2,Math.random()*-2-4,Math.random()*0.2+0.5,1,2,[20,3]);`, `Crumbs.spawnFallingCookie(Game.mouseX,Game.mouseY-32,0,Math.random()*4-2,Math.random()*-2-4,Math.random()*0.2+0.5,'',true,2,100,true,[20,3])`));
	...
	Crumbs.wrinklerEyeObj = {
		...
		anchor: 'top',
		offsetY: -10+Math.sin(Game.realT*0.2+i*3+1.2),
		order: 3,
		scope: 'left',
		behaviors: new Crumbs.behaviorInstance(function(p) {
			if (Game.prefs.notScary && Game.wrinklers[this.parent.wId].close > 0 && Game.AscendTimer == 0) {
				this.offsetY = -10+Math.sin(Game.realT*0.2+this.wId*3+1.2);
				this.imgUsing = Math.sin(Game.realT*0.003+i*11+137+Math.sin(Game.realT*0.017+i*13))>0.9997?1:2;
				this.alpha = Game.wrinklers[this.parent.wId].close;
				return;
			}
			this.imgUsing = 0;
	...
	Crumbs.objectBehaviors.wrinklerMovement = new Crumbs.behavior(function(p) {
		const sw=100+2*Math.sin(Game.realT*0.2+this.wId*3);
		const sh=200+5*Math.sin(Game.realT*0.2-2+this.wId*3);
	...
	Crumbs.objectBehaviors.milkBehavior = new Crumbs.behavior(function() {
		...
		this.getComponent('patternFill').offX = Math.floor((Game.realT*2-(Game.milkH-Game.milkHd)*2000+480*2)%480);
		this.set(toReturn);
	})
	...
	Crumbs.cursorDraw = function(m, ctx) {
		...
		for (var i=0;i<amount;i++)
		{
			...
			var w=0;
			if (fancy) w=(Math.sin(Game.realT*0.025+(((i+n*12)%25)/25)*Math.PI*2));
			if (w>0.997) w=1.5;
	...
	Crumbs.objectBehaviors.dragonDisplayBehavior = new Crumbs.behavior(function() {
		...
		let s=2*(1+Math.sin(Game.realT*0.013)*0.1);
		this.y = -(s*300)/(1.4+0.2*Math.sin(Game.realT*0.01));
		this.scaleX = s;
		this.scaleY = s;
	});
	...
	Crumbs.objectBehaviors.shine1 = new Crumbs.behavior(function() {
		...
		this.rotation = (((Game.realT*0.5)%360)/360)*Math.PI*2;
		let alphaMult = 1;
		var invSpeed = 0;
		if (Game.bgType == 2 || Game.bgType == 4) { alphaMult = 0.5; }
		if (goodBuff) { this.imgUsing = 1; alphaMult = 1; } else if (badBuff) { this.imgUsing = 2; alphaMult = 1; invSpeed = 1; } else { this.imgUsing = 0; }
		var r1=(Game.realT/Game.fps/4)%1;
		if (invSpeed) {r1=1-r1;}
		let scale = (0.5+Math.pow(r1,0.5)) * 512 / 128;
		this.scaleX = scale;
		this.scaleY = scale;
		this.alpha = Math.sin(r1*Math.PI)*alphaMult;
	});
	Crumbs.objectBehaviors.shine2 = new Crumbs.behavior(function() {
		...
		this.rotation = (((Game.realT*0.5)%360)/360)*Math.PI*2+Math.PI;
		let alphaMult = 1;
		var invSpeed = 0;
		if (Game.bgType == 2 || Game.bgType == 4) { alphaMult = 0.5; }
        if (goodBuff) { this.imgUsing = 1; alphaMult = 1; } else if (badBuff) { this.imgUsing = 2; alphaMult = 1; invSpeed = 1; } else { this.imgUsing = 0; }
		...
		if (invSpeed) {r2=1-r2;}
		let scale = (0.5+Math.pow(r2,0.5)) * 512 / 128;
		this.scaleX = scale;
		this.scaleY = scale;
		this.alpha = Math.sin(r2*Math.PI)*alphaMult;
	});
	Crumbs.objectBehaviors.veilMain = new Crumbs.behavior(function() {
		...
		this.rotation = -Game.realT*0.01;
		const scale = (300+Math.sin(Game.realT*0.037)*20)/128;
		this.scaleX = scale;
		this.scaleY = scale;
	});
	...
	Crumbs.objectInits.cookieInit = function() {
		const shine1 = {
			imgs: ['img/flare.png', 'img/flareGold.png', 'img/flareRed.png', 'img/shine.png'],
			id: 'shine1',
			order: -2,
			scaleX: 4,
			scaleY: 4,
    ...
    Crumbs.objectBehaviors.brokenCookieRing = new Crumbs.behavior(function(p) {
        ...
        this.rotation = Game.realT * (p.id == 1 ? 0.007 : -0.002);
	...
	Crumbs.objectBehaviors.cookieShowerBackground = new Crumbs.behavior(function() {
		...
		p.offY = (Math.floor(Game.realT*2)%512);
	});
	...
	Crumbs.objectBehaviors.ascendBackground = new Crumbs.behavior(function(p) {
		...
		if (p.alphaFluctuation) { this.alpha = 0.5*(0.5+Math.sin(Game.realT*0.02)*0.3); }
		...
		this.width = s * 1024;
		this.height = s * 1024;
	}, { fancyRequire: false, alphaFluctuation: false, sMult: function() { return (1+Math.cos(Game.realT*0.0027)*0.05); } });
	Crumbs.initBackground = function() {
		...
		Crumbs.spawn({
			...
			//alpha: 0.5,
			components: new Crumbs.component.patternFill(),
			behaviors: new Crumbs.behaviorInstance(Crumbs.objectBehaviors.ascendBackground, { fancyRequire: true, alphaFluctuation: true, sMult: function() { return 2*(1+Math.sin(Game.realT*0.002)*0.07); } })
		});
	}
	...
	Crumbs.objectBehaviors.petDisplayMove = new Crumbs.behavior(function(p) {
		...
		this.offsetX = Game.specialTab == p.tab ? 0 : Math.sin(Game.realT * 0.2 + this.parent.placement) * 3;
		this.offsetY = -(Game.specialTab == p.tab ? 6 : Math.abs(Math.cos(Game.realT * 0.2 + this.parent.placement)) * 6);
	}, { tab: '' });
	...
	Crumbs.objectBehaviors.nebulaSpin1 = new Crumbs.behavior(function() {
		this.rotation = Game.realT*0.001;
		let s = (600+150*Math.sin(Game.realT*0.007))*Game.AscendZoom / 128;
		this.scaleX = s;
		this.scaleY = s;
	});
	Crumbs.objectBehaviors.nebulaSpin2 = new Crumbs.behavior(function() {
		this.rotation = -Game.realT*0.0007;
		let s = (600+150*Math.sin(Game.realT*0.0037))*Game.AscendZoom / 128;
		this.scaleX = s;
		this.scaleY = s;
	});
	...
	Crumbs.initNebulaConservative = function() {
		Crumbs.spawn({
			...
			components: [new Crumbs.component.settings({ globalCompositeOperation: 'lighter' }), new Crumbs.component.canvasManipulator({ function: function(m, ctx) { 
				...
				ctx.globalCompositeOperation = 'lighter';
				ctx.globalAlpha = 0.5;
				ctx.rotate(Game.realT * 0.001);
				let s = (600 + 150 * Math.sin(Game.realT * 0.007)) * Game.AscendZoom;
				ctx.drawImage(Pic('heavenRing1.jpg'), -s / 2, -s / 2, s, s);
				ctx.rotate(-Game.realT * 0.0017);
		...
		for (var i in Game.wrinklers)
		{
			if (Game.wrinklers[i].selected)
			{
				...
				for (var i=0;i<Math.floor(dist/12);i++)
				{
					...
					ctx.beginPath();
					ctx.arc(xC,yC,4+(Game.prefs.fancy?2*Math.pow(Math.sin(-Game.realT*0.2+i*0.3),4):0),0,2*Math.PI,false);
					ctx.fill();
					ctx.stroke();
				}
				...
				var s=54+2*Math.sin(Game.realT*0.4);
